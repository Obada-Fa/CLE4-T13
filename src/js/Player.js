...
class Player extends Actor {
  ...
  onPreUpdate(engine, delta) {
    ...
    if (this.healthBar) {
      this.healthBar.pos = new Vector(
        0,
        (-this.height * this.scale.y) / 2 - 10
  ...
  _constrainPosition(delta) {
    ...
    const newX = Math.max(
      this.minX + halfWidth,
      Math.min(this.maxX - halfWidth, this.pos.x + (this.vel.x * delta) / 1000)
    );
    const newY = Math.max(
      this.minY + halfHeight,
      Math.min(this.maxY - halfHeight, this.pos.y + (this.vel.y * delta) / 1000)
    );
